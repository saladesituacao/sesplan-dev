        ...
        <table class="table table-striped" cellspacing="0" cellpadding="0">
            ...
            <tbody>
                ...
                $sql_regiao = "SELECT tb_hospital.* FROM tb_hospital INNER JOIN tb_indicador_hospital ON tb_indicador_hospital.cod_hospital =   
                            tb_hospital.cod_hospital WHERE tb_hospital.cod_tipo = 2 AND tb_hospital.cod_ativo = 1 AND tb_indicador_hospital.cod_chave = ".$id."  
                ...
                while ($rs1 = pg_fetch_array($q)) { 
                    ...
                    $cod_hosp_qtd_conv += 1;
                    $cod_hosp_ind_conv .= "[".$cod_hospital."]";
                }
                $cod_hosp_ind_conv = str_replace('][', ',', $cod_hosp_ind_conv);
                $cod_hosp_ind_conv = str_replace(']', '', $cod_hosp_ind_conv);
                $cod_hosp_ind_conv = str_replace('[', '', $cod_hosp_ind_conv);
                ?>    
                <input type="hidden" id="cod_hosp_qtd_conv" value="<?=$cod_hosp_qtd_conv?>" />
                <input type="hidden" id="cod_hosp_ind_conv<?=$ct_mes?>" value="<?=$cod_hosp_ind_conv?>" />                                                                                    
            </tbody>
        </table>                                        
    </div> 
</div>