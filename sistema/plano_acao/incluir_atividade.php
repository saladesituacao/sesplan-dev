...
if(!empty($id)) {
    ...
    $q1 = pg_query("SELECT * FROM tb_atividade_plano_acao WHERE cod_atividade_plano_acao = '$id'");
    $rs1 = pg_fetch_array($q1);

    $nr_atividade = trim($rs1['nr_atividade']);
    $txt_atividade = trim($rs1['txt_atividade']);    		
    ...
    $nr_prazo = trim($rs1['nr_prazo']);
    $txt_unidade_prazo = trim($rs1['txt_unidade_prazo']);
}	
else 
{
...
<div id="main" class="container-fluid">
    <?php if(empty($id)) {
    ?>
        <h3 class="page-header">Atividade > Incluir</h3>
    <?php
    } 
    else {
    ?>
        <h3 class="page-header">Atividade > Alterar</h3>
    <?php
    }
    ?>   
    <form id="frm1" action="manter_atividade.php">
        ...
        <div class="row">
            <div class="form-group col-md-6">
                <label for="exampleInputEmail1">Nome/Número da Atividade:</label>
                <input type="text" class="form-control" id="nr_atividade" name="nr_atividade" placeholder="Obrigatório" value="<?=$nr_atividade?>">
            </div>
        </div>
        ...
        <div class="row">
            <div class="form-group col-md-6">
                ...
                <textarea class="form-control" rows="5" id="txt_atividade" name="txt_atividade" placeholder="Obrigatório"><?=$txt_atividade?></textarea>
            </div>	 
        ...
        <div class="row">
            ...
            <div class="col-md-6">                        
                ...
                ?>                  	  	
                <a href="listar_atividade.php?id=<?php echo $idPlanoAcao ?>" class="btn btn-default">Voltar</a>
            </div>
        </div>
    </form>
</div>
...
function popularDataFinal(data){



    if (frm1.dt_inicial.value != ''){

        if (frm1.nr_prazo.value != ''){

            if (frm1.cod_unidade_prazo.value != ''){
                var dt = new Date();
                dt.addDias(frm1.nr_prazo.value);
